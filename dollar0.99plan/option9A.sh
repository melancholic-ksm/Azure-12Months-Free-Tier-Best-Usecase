...
# 
# OPTION A: B2ats v2 (AMD x64) + B2pts v2 (ARM64)
#           Total: 4 vCPUs, 2 GB RAM
#===============================================================================
#
...
#   • powr-vm (B2pts v2, ARM64, 2 vCPU, 1GB RAM)
#
...
# powr-nic for ARM64 VM (private only)
az network nic create \
  --resource-group "$RG" \
  --name powr-nic \
  --vnet-name "$VNET_NAME" \
...
echo "  Created: mgmt-nic (public), powr-nic (private)"
echo ""
...
echo "  Creating $VM_POWR (Standard_B2pts_v2, ARM64)..."
az vm create \
  --resource-group "$RG" \
  --name "$VM_POWR" \
  --nics "$POW_NIC_ID" \
...
az vm wait --resource-group "$RG" --name "$VM_POWR" --created
...
az vm boot-diagnostics enable --resource-group "$RG" --name "$VM_POWR" --storage "$STORAGE_URI"
...
echo "  • $VM_POWR (Standard_B2pts_v2, ARM64, 64GB P6 SSD) - Private IP only"
echo ""
echo "=============================================="
